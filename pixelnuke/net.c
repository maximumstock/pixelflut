...
void handle_size_command(uv_stream_t *stream, ssize_t nread, const uf_buf_t *buf) {
...
void handle_stats_command(uv_stream_t *stream, ssize_t nread, const uf_buf_t *buf) {
	uv_write_t *req = (uv_write_t *)malloc(sizeof(uv_write_t));
...
void handle_help_command(uv_stream_t *stream, ssize_t nread, const uf_buf_t *buf) {
	uv_write_t *req = (uv_write_t *)malloc(sizeof(uv_write_t));
 ...
 *
 * 1. If the first command is SIZE, HELP or STATS, we just reply directly and ignore the remaining command buffer
 *
 * For PX getting and setting, we want to be able to answer multiple commands together, so:
 *
 * 2. If the first command is get PX, we parse the command bufferuntil we encounter something else, stop parsing,
 *    prepare the output buffer(s) and send those out. We can then return control flow and let the command parsing
 *    restart.
 *
 * 3. If the first command is set PX, we can do the same as in #2, but we don't need to write anything back.
 *
 * 4. Error Handling: If nothing matches we just ignore the remaining buffer for now. In the future we can respond with
 *    an actual parser error message.
 */
void on_read(uv_stream_t *stream, ssize_t nread, const uv_buf_t *buf)
{
  ...
  // We can actually send multiple buffers https://ant.readthedocs.io/en/latest/stream.html#c.uv_write
	...
	if (fast_str_startswith("SIZE", start)) {
